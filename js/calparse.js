...
var GCal = {
  ...
  init: function() {
    // GCal.service = new google.gdata.calendar.CalendarService('konaSignage-app');
  }, 
  
  kona_schedule: "or2b5kqjl58ndf8ien8atkcplg@group.calendar.google.com",
  kona_events: "o2jfjbba7bcrhkk5p3scfqj7vc@group.calendar.google.com",
  mauka_theater: "oebubbrli984mh257hd986td80@group.calendar.google.com",
  ...
  get_calendar: function (name, range, callback, failcount) {
    ...
    var key = "AIzaSyDQOhBkru--nRMex2ZrJOw7ByX0PQKdKNA";
    // var uri = "https://www.google.com/calendar/feeds/" + name + "/public/full-noattendees";
    var uri = "https://www.googleapis.com/calendar/v3/calendars/" + name + "/events?key=" + key;    
    
    // var query = new google.gdata.calendar.CalendarEventQuery(uri);
    var startMin, startMax, timeOffset, timezone;
    var query = {};
    ...
    case "upcoming":
      ...
      break;
    default:
      time = [today, today.clone().addDays(range)];
    }
    ...
    query["timeMin"] = GCal.format_date(startMin);
    query["timeMax"] = GCal.format_date(startMax);
    
    // uri += "&orderBy=startTime";
    // uri += "&singleEvents=true";
    // // 2012-04-08T23:59:00-10:00
    // timeOffset = time[0].getTimezoneOffset() / 60;
    // // set the proper +/- for the timeOffset
    // timezone = -timeOffset > 0 ? "+" : "-";
    // // convert timeOffset to 2 digit number
    // if(timeOffset < 10) {timeOffset = "0" + timeOffset};
    // // console.log(timezone + timeOffset + ":00");
    // uri += "&timeMin=" + encodeURIComponent(startMin.toString("yyyy-MM-ddThh:mm:00") + timezone + timeOffset + ":00");
    // uri += "&timeMax=" + encodeURIComponent(startMax.toString("yyyy-MM-ddThh:mm:00") + timezone + timeOffset + ":00");
    ...
    req.success(function (data) {
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        // var eventTitle = eventEntry.getTitle().getText();
        // console.log('Event title = ' + eventTitle);
        // console.log(eventEntry.start.dateTime);
        events.push(new Event({
          title: eventEntry.summary,
          start_time: Date.parse(eventEntry.start.dateTime.substring(0,19)), // Dumb date parsing does NOT count in time zones
          end_time: Date.parse(eventEntry.end.dateTime.substring(0,19)),
  ...
  format_date: function(date) {
    ...
    if(timeOffset < 10) {timeOffset = "0" + timeOffset};
    // console.log(timezone + timeOffset + ":00");
    return date.toString("yyyy-MM-ddThh:mm:00") + timezone + timeOffset + ":00";
  }
};